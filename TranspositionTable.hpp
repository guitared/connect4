...
constexpr uint64_t next_prime(uint64_t n) {return has_factor(n, 2, n) ? next_prime(n + 1) : n;}
...
class TranspositionTable : public TableGetter<key_t, value_t> {
 private:
  ...
  size_t index(key_t key) const {return key % size;}

 public:
  TranspositionTable() {K = new partial_key_t[size]; V = new value_t[size]; reset();}
  ~TranspositionTable() {delete[] K; delete[] V;}

  void reset() {memset(K, 0, size * sizeof(partial_key_t)); memset(V, 0, size * sizeof(value_t));}
  void put(key_t key, value_t value) {size_t pos = index(key); K[pos] = key; V[pos] = value;}

  value_t get(key_t key) const override {size_t pos = index(key); if(K[pos] == (partial_key_t)key) return V[pos]; else return 0;}
};
